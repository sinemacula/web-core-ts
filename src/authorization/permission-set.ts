 ...
 *
 * The kernel's only authorization primitive: a flat list of granted
 * permission strings with wildcard-prefix support. Role models, permission
 * hierarchies, and where the granted list comes from (a user record, a
 * membership, an API token) are all app concerns layered on top of this.
 *
 ...
 *
 * Wildcard grammar: a granted entry ending in `.*`, `:*`, or the bare `*`
 * acts as a prefix grant.
 *
 * - `'*'` allows every permission.
 * - `'users.*'` allows `'users.view'` and `'users.edit.self'`, but not
 *   `'usersx.view'` - the wildcard only matches at the declared prefix
 *   boundary (the character preceding `*` is part of the match).
