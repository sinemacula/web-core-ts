 ...
 *
 * A deploy removes the previous build's hashed chunk files, so a tab left
 * open since before that deploy fails to lazy-load any route it has not
 * already visited. The update monitor warns about this proactively by
 * polling the deployed version; this module recovers from it reactively,
 * from inside the router's own error channel, catching the tab that missed
 * the warning or navigated before it was shown.
 *
 * A chunk-load failure is treated as recoverable once per target path within
 * a rolling window: the first failure triggers a full document reload, which
 * fetches the fresh `index.html` and the chunks it now references. A second
 * failure for the same path inside that window means the reload did not cure
 * it - a genuinely broken deploy - so the failure is reported instead of
 * retried forever.
 *
 ...
 * @returns true when the error is an `Error` whose message matches a known chunk-load failure
 */
