 ...
 * Subsequent calls to {@link ConfirmService.confirm} while a request is
 * already active are queued (FIFO) and surfaced automatically once the
 * current request is settled via {@link ConfirmService.settle}.
 *
 ...
 * {@link cancelLabel} are translation keys; the rendering host translates
 * them before display.
 */
...
/**
 * A {@link ConfirmRequest} that has been assigned an id and is currently
 * active (i.e. waiting for the user to settle it).
 */
 ...
 * a destructive action requires explicit user confirmation. Mount a single
 * host component that reads {@link active} to render the modal.
 */
export class ConfirmService {
     ...
     *
     * Resolves the associated promise with the given outcome, then surfaces
     * the next queued request (if any). No-op when there is no active request.
     *
