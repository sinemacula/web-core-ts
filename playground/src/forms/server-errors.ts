 ...
 * submit handler. Server messages are display-ready English text from the API,
 ...
 *
 * Builds a `{ field: firstMessage }` map from every field that has at least one
 * message, skipping fields whose message array is empty, and applies it with a
 * single `setErrors` call.
 *
 * @param error - the thrown failure to inspect
 * @param setErrors - applies the mapped field errors to the form
 * @returns true when at least one field message was applied to the form; false
 * when `error` is not a validation failure, or the failure carries an error map
 * with no field messages - callers should fall through to their generic error
