 ...
 *
 * vee-validate 4.15 does not implement the Standard Schema contract, so a
 * zod object schema cannot be handed to `useForm`'s `validationSchema` option
 * directly. This module is the minimal inline adapter that bridges a zod
 * object schema to vee-validate's `TypedSchema` contract, so every form
 * composable in the app validates against zod without a third-party bridge
 * package.
 *
 ...
 * vee-validate 4. Runs validation via `safeParse`; the first issue per path
 * is forwarded as the field error so the message string is the translation
 * key.
 *
...
export function zodToTypedSchema<T extends z.ZodObject<z.ZodRawShape>>(
    ...
    return {
        // biome-ignore lint/style/useNamingConvention: vee-validate TypedSchema discriminator
        ...
        parse(values: z.input<T>): Promise<{ value?: z.output<T>; errors: TypedSchemaError[] }> {
            ...
            for (const issue of result.error.issues) {
                ...
                if (!seen.has(dotPath)) {
                    seen.add(dotPath);
                    // exactOptionalPropertyTypes: include `path` only when non-empty.
